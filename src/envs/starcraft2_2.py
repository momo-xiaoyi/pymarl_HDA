...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        # Map arguments
        if sight_view:
            self.sight_view = 9 #default 9
            self.sight_view_index = True 
        else:
            self.sight_view = 1e-10
            self.sight_view_index = False
    ...
    def get_obs_agent_all(self, agent_id):
        """Returns observation for agent_id. The observation is composed of:
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            y = unit.pos.y
            dis_norm = 9
            if self.sight_view_index:
                sight_range = 1000 #see_all
            else:
                sight_range = 9  #see more
            #sight_range = 1e-8
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                if (
                    ...
                    enemy_feats[e_id, 1] = dist / dis_norm  # distance
                    enemy_feats[e_id, 2] = (
                        e_x - x
                    ) / dis_norm  # relative X
                    enemy_feats[e_id, 3] = (
            ...
            for i, al_id in enumerate(al_ids):
                ...
                if (
                    ...
                    ally_feats[i, 0] = 1  # visible
                    ally_feats[i, 1] = dist / dis_norm  # distance
                    ally_feats[i, 2] = (al_x - x) / dis_norm  # relative X
                    ally_feats[i, 3] = (al_y - y) / dis_norm  # relative Y
    ...
    def get_obs_all(self):
        ...
        during decentralised execution.
        """
        agents_obs = [self.get_obs_agent_all(i) for i in range(self.n_agents)]
        return agents_obs
    
    def get_obs_size_all(self):
        """Returns the size of the observation."""
        own_feats = self.get_obs_own_feats_size() 
        move_feats = self.get_obs_move_feats_size()

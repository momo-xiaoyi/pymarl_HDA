...
class PGLearner_v2:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        advantages, td_error, targets_taken, log_pi_taken, entropy, mac_out_all = self._calculate_advs(batch, rewards, terminated, actions, avail_actions,
        ...
        mask_teach = _mask.unsqueeze(-1).expand_as(teach_error)
        ...
        coma_loss = pg_loss + self.args.vf_coef * vf_loss #+ loss_teach
    ...
    def _calculate_advs(self, batch, rewards, terminated, actions, avail_actions, mask, bs, max_t):
        ...
        for t in range(batch.max_seq_length):
            _, q_out = self.mac_all.forward(batch, t=t)
            q_outs.append(q_out)
        q_outs = th.stack(q_outs, dim=1)  # Concat over time
        ...
        self.mac.init_hidden(batch.batch_size)
        for t in range(batch.max_seq_length):
            agent_out, _ = self.mac.forward(batch, t=t)
            mac_out.append(agent_out)
        mac_out = th.stack(mac_out, dim=1)  # Concat over time
        ...
        return advantages, td_error, targets_taken[:, :-1].unsqueeze(2).repeat(1, 1, self.n_agents, 1).reshape(-1), log_pi_taken, entropy, mac_out

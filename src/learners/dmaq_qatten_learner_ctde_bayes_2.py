...
class DMAQ_qattenLearner:
    ...
    def sub_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, mac, mixer, optimiser, params,
        ...
        # Calculate estimated Q-Values with state
        mac_out_all = []
        mac.init_hidden(batch.batch_size)
        for t in range(batch.max_seq_length):
            agent_outs_all = mac.forward_all(batch, t=t)
            mac_out_all.append(agent_outs_all)
        ...
        """
        chosen_action_qvals = th.gather(mac_out_all[:, :-1], dim=3, index=actions).squeeze(3)  # Remove the last dim
        x_mac_out = mac_out_all.clone().detach()
        ...
        # Calculate the Q-Values necessary for the target with state
        target_mac_out_all = []
        self.target_mac.init_hidden(batch.batch_size)
        for t in range(batch.max_seq_length):
            target_agent_outs_all = self.target_mac.forward_all(batch, t=t)
            target_mac_out_all.append(target_agent_outs_all)
        # We don't need the first timesteps Q-Value estimate for calculating targets
        target_mac_out_all = th.stack(target_mac_out_all[1:], dim=1)  # Concat across time
        # Mask out unavailable actions
        target_mac_out_all[avail_actions[:, 1:] == 0] = -9999999
        target_mac_out = target_mac_out_all
        ...
        central_target_max_agent_qvals = th.gather(central_target_mac_out[:,1:], 3, cur_max_actions[:,:].unsqueeze(4).repeat(1,1,1,1,self.args.central_action_embed)).squeeze(3)
        # ---
        target_max_qvals_central = self.target_central_mixer(central_target_max_agent_qvals, batch["state"][:,1:])
        ...
        # Calculate 1-step Q-Learning targets
        targets = rewards + self.args.gamma * (1 - terminated) * target_max_qvals_central
        ...
        central_masked_td_error = central_td_error * mask

        # Normal L2 loss, take mean over actual data
        if self.args.mixer == "dmaq_qatten":
            loss = (masked_td_error ** 2).sum() / mask.sum() + q_attend_regs
            loss = loss + (central_masked_td_error ** 2).sum() / mask.sum()
        else:
            loss = (masked_td_error ** 2).sum() / mask.sum()
            loss = loss + (central_masked_td_error ** 2).sum() / mask.sum()
            
        mac_out_all_detach = mac_out_all.clone().detach()
        mac_out_all_detach[avail_actions == 0] = -9999999
        cur_max_actions_all = mac_out_all_detach.max(dim=3, keepdim=True)[1]
        mac_out_all_onehot = th.zeros_like(mac_out).scatter_(1, cur_max_actions_all, 1)
        bayes_error = (mac_out[:, :-1] - mac_out_all_onehot[:, :-1].detach()).abs()
        mask_bayes = mask[:, :, :, np.newaxis].expand_as(bayes_error)
        masked_bayes_error = bayes_error * mask_bayes
        loss = loss + (masked_bayes_error).sum() / mask_bayes.sum()

...
class MAXQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        qmix_loss_mix_dif = ((ws.detach()-1)*(masked_td_error_mix ** 2)).sum() / mask.sum()
        qmix_loss = ((masked_td_error ** 2)).sum() / mask.sum()

        # The weightings for the different losses aren't used (they are always set to 1)
        #loss_mix = self.args.qmix_loss * qmix_loss_mix + self.args.central_loss * central_loss
        
        loss = self.args.qmix_loss * (qmix_loss + qmix_loss_mix_dif) + self.args.central_loss * central_loss 

...
class MAXQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            agent_outs, pi = self.mac.forward(batch, t=t)
            mac_out.append(agent_outs)
            pi_out.append(pi)
        mac_out = th.stack(mac_out, dim=1)  # Concat over time
        pi_out = th.stack(pi_out, dim=1)  # Concat over time
        
        # choose pi
        chosen_action_pi = th.gather(pi_out[:, :-1], dim=3, index=actions).squeeze(3)  # Remove the last dim
        action_mask = mask.repeat(1, 1, self.args.n_agents)
        chosen_action_pi[action_mask == 0] = 1.0
        log_chosen_action_pi = th.log(chosen_action_pi)
        ...
        for t in range(batch.max_seq_length):
            target_agent_outs, pi = self.target_mac.forward(batch, t=t)
        ...
        # explore gradient
        #explore_error = (targets - chosen_action_qvals).repeat(1, 1, self.args.n_agents).detach() * log_chosen_action_pi
        #masked_explore_error = explore_error * mask
        entropy = categorical_entropy(pi_out[:,:-1])
        entropy[action_mask == 0] = 0
        entropy_loss = (entropy * action_mask).sum() / action_mask.sum()
        ...
        # explore gradient
        explore_error = (targets - central_chosen_action_qvals).repeat(1, 1, self.args.n_agents).detach() * log_chosen_action_pi
        masked_explore_error = (explore_error * mask).sum() / mask.sum()
        ...
        qmix_loss = (ws.detach()*(masked_td_error ** 2)).sum() / mask.sum() - masked_explore_error - 0.01 * entropy_loss / entropy_loss.item() # add explore error

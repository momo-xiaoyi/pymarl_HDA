...
from modules.mixers.head import HEAD
...
class MAXQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer is not None:
            ...
            self.params_mix = list(self.mixer.parameters())
            self.target_mixer = copy.deepcopy(self.mixer)

        # a little wasteful to deepcopy (e.g. duplicates action selector), but should work for any MAC
        self.target_mac = copy.deepcopy(mac)
        ##########################
        self.target_head = HEAD(args)
        ##########################
        ...
        if self.args.central_mixer in ["ff", "atten"]:
            if self.args.central_loss == 0:
                ...
                ##########################
                self.central_head = self.head
                ##########################
                self.target_central_mac = self.target_mac
                ##########################
                self.target_central_head = self.target_head
                ##########################
            else:
                ...
                ##########################
                self.central_head = HEAD(args)
                ##########################
                self.target_central_mac = copy.deepcopy(self.central_mac)
                ##########################
                self.target_central_head = HEAD(args)
                ##########################
                self.params += list(self.central_mac.parameters())
                self.params += list(self.central_head.parameters())
        else:
        ...
        #self.optimiser_mix = RMSprop(params=self.params_mix, lr=args.lr, alpha=args.optim_alpha, eps=args.optim_eps)
        self.optimiser = Adam(params=self.params, lr=args.lr*10)
        self.optimiser_mix = Adam(params=self.params_mix, lr=args.lr)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            ...
            agent_outs = self.head(agent_outs, batch["state"][:, t], batch)
        ...
        for t in range(batch.max_seq_length):
            ...
            target_agent_outs = self.target_head(target_agent_outs, batch["state"][:, t], batch)
        ...
        for t in range(batch.max_seq_length):
            ...
            #agent_outs = self.central_head(agent_outs, batch["state"][:, t], batch).unsqueeze(3)
        ...
        for t in range(batch.max_seq_length):
            ...
            #target_agent_outs = self.target_central_head(target_agent_outs, batch["state"][:, t], batch).unsqueeze(3)
        ...
        # Mix
        copy_inputs = chosen_action_qvals.detach()
        chosen_action_qvals = self.mixer(chosen_action_qvals, batch["state"][:, :-1])
        chosen_action_qvals_mix = self.mixer(copy_inputs, batch["state"][:, :-1])
        ...
        td_error_mix = (chosen_action_qvals_mix - (targets.detach()))

        mask = mask.expand_as(td_error)

        # 0-out the targets that came from padded data
        masked_td_error = td_error * mask
        masked_td_error_mix = td_error_mix * mask
        ...
        self.optimiser_mix.zero_grad()
        loss_mix.backward()
        grad_norm = th.nn.utils.clip_grad_norm_(self.params_mix, self.args.grad_norm_clip)
        self.optimiser_mix.step()
    ...
    def cuda(self):
        ...
        if self.central_mac is not None:
            ...
            self.central_head.cuda()
            self.target_central_mac.cuda()
            self.target_central_head.cuda()

...
class IQNLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.params += list(self.mac_all.parameters())
        ...
        self.target_mac_all = copy.deepcopy(self.mac_all)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            agent_outs, agent_rnd_quantiles = self.mac_all.forward(batch, t=t, forward_type="policy")
        ...
        target_mac_out = []
        self.target_mac_all.init_hidden(batch.batch_size)
        for t in range(batch.max_seq_length):
            target_agent_outs, _ = self.target_mac_all.forward(batch, t=t, forward_type="target")
        ...
        for t in range(batch.max_seq_length):
            agent_outs, _ = self.mac.forward(batch, t=t, forward_type="policy")
            assert agent_outs.shape == (batch.batch_size * self.args.n_agents, self.args.n_actions, self.n_quantiles)
            #assert agent_rnd_quantiles.shape == (batch.batch_size * n_quantile_groups, self.n_quantiles)
            #agent_rnd_quantiles = agent_rnd_quantiles.view(batch.batch_size, n_quantile_groups, self.n_quantiles)
            #rnd_quantiles.append(agent_rnd_quantiles)
            agent_outs = agent_outs.view(batch.batch_size, self.args.n_agents, self.args.n_actions, self.n_quantiles)
            mac_out_ori.append(agent_outs)
        mac_out_ori = th.stack(mac_out_ori, dim=1) # Concat over time
        
        # Max over target Q-Values
        if self.args.double_q:
            # Get actions that maximise live Q (for double q-learning)
            mac_out_detach = mac_out_ori.clone().detach()
        ...
        #---------------*******-------------
        mac_out_all =  mac_out.clone().detach()
        #target_central_mac_out_clone =  central_mac_out.clone().detach()
        mac_out_all[avail_actions == 0] = -9999999
        #print(mac_out_all.shape)
        #print(mac_out_all.mean(dim=4).shape)
        cur_max_actions_all = mac_out_all.mean(dim=4).max(dim=-1, keepdim=True)[1].squeeze(-1)
        #print(cur_max_actions_all.shape)
        #cur_max_actions_all = cur_max_actions_all.unsqueeze(-1).expand(-1, -1, -1, self.n_target_quantiles)
        #print(cur_max_actions_all.shape)
        mac_out_all_onehot = F.one_hot(cur_max_actions_all, num_classes=avail_actions.shape[-2])
        mac_out_all_onehot = mac_out_all_onehot.unsqueeze(-1).expand(-1, -1, -1, -1, self.n_target_quantiles)
        #print(mac_out_all_onehot.shape,"#",mac_out_ori.shape)
        # teach_error
        teach_error = (mac_out_ori[:, :-1] - mac_out_all_onehot[:, :-1].detach())
        mask_teach = mask.unsqueeze(-1).unsqueeze(-1).expand_as(teach_error)
        # 0-out the targets that came from padded data
        masked_teach_error = teach_error * mask_teach
        loss_teach = (masked_teach_error ** 2).sum() / mask_teach.sum() 
        #---------------*******-------------
        ...
        loss = loss.sum() / mask.sum() + loss_teach
        assert loss.shape == ()
    ...
    def _update_targets(self):
        ...
        self.target_mac_all.load_state(self.mac_all)
        if self.mixer is not None:

...
class DMAQ_qattenLearner:
    ...
    def sub_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, mac, mixer, optimiser, params,
        ...
        for t in range(batch.max_seq_length):
            target_agent_outs = self.target_mac_all.forward(batch, t=t)
            target_mac_out_all.append(target_agent_outs)

        target_mac_out_all_save = th.stack(target_mac_out_all, dim=1)  # Concat across time
        ...
        if self.args.double_q:
            ...
            target_chosen_qvals = th.gather(target_mac_out_all, 3, cur_max_actions).squeeze(3)
            target_max_qvals = target_mac_out_all.max(dim=3)[0]
        ...
        cur_max_actions_all = mac_out_all.max(dim=-1, keepdim=True)[1].squeeze(-1)
        mac_out_all_onehot = F.one_hot(cur_max_actions_all, num_classes=avail_actions.shape[-1])
        # teach_error
        teach_error = (mac_out[:, :-1] - mac_out_all_onehot[:, :-1].detach())
        mask_teach = mask.unsqueeze(-1).expand_as(teach_error)
        ...
        if self.args.mixer == "dmaq_qatten":
            loss = (masked_td_error ** 2).sum() / mask.sum() + q_attend_regs + loss_teach
        else:
            loss = (masked_td_error ** 2).sum() / mask.sum() + loss_teach

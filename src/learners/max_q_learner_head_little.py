...
class MAXQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Mix
        mix_inputs = chosen_action_qvals.detach()
        chosen_action_qvals = self.mixer(chosen_action_qvals, batch["state"][:, :-1])
        chosen_action_qvals_mix = self.mixer(mix_inputs, batch["state"][:, :-1])
        ...
        qmix_loss_mix = ((ws.detach()-1)*(masked_td_error_mix ** 2)).sum() / mask.sum()

        # The weightings for the different losses aren't used (they are always set to 1)
        loss = self.args.qmix_loss * (qmix_loss + qmix_loss_mix) + self.args.central_loss * central_loss

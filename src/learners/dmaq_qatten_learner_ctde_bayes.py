...
class DMAQ_qattenLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.target_mac_2 = copy.deepcopy(mac)
        self.last_target_update_episode_2 = 0
    ...
    def sub_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, mac, mixer, optimiser, params,
        ...
        for t in range(batch.max_seq_length):
            target_agent_outs = self.target_mac.forward_all(batch, t=t)
            target_mac_out.append(target_agent_outs)
        target_mac_out_all = th.stack(target_mac_out, dim=1)  # Concat across time
            
        mac_out_all_detach = target_mac_out_all.clone().detach()
        mac_out_all_detach[avail_actions == 0] = -9999999
        cur_max_actions_all = mac_out_all_detach.max(dim=3, keepdim=True)[1]
        mac_out_all_onehot = th.zeros_like(mac_out).scatter_(3, cur_max_actions_all, 1)
        ...
        loss = loss + (masked_bayes_error**2).sum() / mask_bayes.sum() #* 12.0
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, show_demo=False, save_data=None):
        ...
        if (episode_num - self.last_target_update_episode_2) / self.args.target_update_interval_2 >= 1.0:
            self._update_targets_2()
            self.last_target_update_episode_2 = episode_num
    ...
    def _update_targets_2(self):
        self.target_mac_2.load_state(self.mac)

    def cuda(self):
        self.mac.cuda()
        self.target_mac.cuda()
        self.target_mac_2.cuda()
        if self.mixer is not None:
            self.mixer.cuda()

...
def run(_run, _config, _log, pymongo_client=None):
    ...
    if args.use_tensorboard:
        if args.env_args['sight_view']:
            sight_arg = "sight_9"
        else:
            sight_arg = "sight_0"
        tb_logs_direc = os.path.join(dirname(dirname(dirname(abspath(__file__)))), "results", "tb_logs", sight_arg, args.env_args['map_name'])
...
def evaluate_sequential(args, runner, mac):

    for _ in range(args.test_nepisode):
        runner.run(mac=mac, test_mode=True)
...
def run_sequential(args, logger):
    ...
    scheme = {
        ...
        "obs_all": {"vshape": env_info["obs_shape_all"], "group": "agents"},
        "actions": {"vshape": (1,), "group": "agents", "dtype": th.long},
        "choose_head": {"vshape": (1,), "group": "agents", "dtype": th.long},
    ...
    if args.checkpoint_path != "":
        ...
        if args.if_lower==False:
            runner.t_env = timestep_to_load

        if args.evaluate or args.save_replay:
            evaluate_sequential(args, runner, mac)
            return
    ...
    while runner.t_env <= args.t_max:
        ...
        episode_batch = runner.run(mac=mac, test_mode=False)

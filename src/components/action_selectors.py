...
class EpsilonGreedyActionSelector_explore():
    ...
    def select_action(self, agent_inputs, avail_actions, pi, t_env, test_mode=False):
        ...
        # explore 
        max_actions = masked_q_values.max(dim=2, keepdims=True)[1]
        #avail_actions = avail_actions.scatter_(1, max_actions, 0) # without max action
        pi[masked_q_values == masked_q_values.max(dim=2, keepdims=True)[0]] = 0.001 # without max action, somehow may only max action available
        pi[avail_actions == 0] = 0 # explore with pi
        
        random_actions = Categorical(pi).sample().long()  # use pi to explore

        picked_actions = pick_random * random_actions + (1 - pick_random) * masked_q_values.max(dim=2)[1]
        return picked_actions


REGISTRY["epsilon_greedy_explore"] = EpsilonGreedyActionSelector_explore
